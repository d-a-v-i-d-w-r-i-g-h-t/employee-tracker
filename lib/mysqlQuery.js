// use mysql 2 module with promise support
const mysql = require("mysql2/promise");

// connect to the database and send mysql query
async function mysqlQuery(queryOption, optionalParams) {

  // scope query outside of switch block
  let query;

  // switch case to select between query options
  switch(queryOption) {
  ...
  try {
    ...
    // scope results, addResults outside if/else block
    let results = [];
    let addResults;

    // add, modify, and delete queries include optional parameters in the query
    if (optionalParams) {
      [addResults] = await connection.query(query, optionalParams);
      // console.log(addResults);

    // select queries return table results
    } else {
    ...
    // cleanedData will be used to display data tables
    return cleanedData;
